<template>
  ...
  <div class="input-section">
    <div class="input-grid">
      ...
      <div class="input-item">
        <div class="input-label">
          <label><span class="required-mark">*</span>联系人：</label>
          
          <div v-if="validationStatus.contact.isError" class="error-message">{{ validationStatus.contact.message }}</div>
        </div>
        <div class="input-area" :class="{ 'error-border': validationStatus.contact.isError }">
          <el-input v-model="props.table.projectInfo.contact" size="medium"
            :disabled="isOwner != undefined && !isOwner" @input="validateChineseEnglishInput($event, 'contact')"></el-input>
        </div>
      </div>
      <div class="input-item">
        <div class="input-label">
      ...
      <div class="input-item">
        <div class="input-label">
          <label><span class="required-mark">*</span>合作意向项目：</label>
          
          <div v-if="validationStatus.coIntentionalProject.isError" class="error-message">{{ validationStatus.coIntentionalProject.message }}</div>
        </div>
        <div class="input-area" :class="{ 'error-border': validationStatus.coIntentionalProject.isError }">
          <el-input v-model="props.table.projectInfo.coIntentionalProject" size="medium"
            :disabled="isOwner != undefined && !isOwner" @input="validateChineseEnglishInput($event, 'coIntentionalProject')"></el-input>
        </div>
      </div>
      <div class="input-item">
        <div class="input-label">
          <label><span class="required-mark">*</span>合作意向单位：</label>
          
          <div v-if="validationStatus.coIntentionalUnit.isError" class="error-message">{{ validationStatus.coIntentionalUnit.message }}</div>
        </div>
        <div class="input-area" :class="{ 'error-border': validationStatus.coIntentionalUnit.isError }">
          <el-input v-model="props.table.projectInfo.coIntentionalUnit" size="medium"
            :disabled="isOwner != undefined && !isOwner" @input="validateChineseEnglishInput($event, 'coIntentionalUnit')"></el-input>
        </div>
      </div>
      <div class="input-item">
        <label>项目初步预算金额（万元）：</label>
        <div class="input-area">
          <el-input v-model="props.table.projectInfo.budgetMoney" size="medium" type="number" :min="0"
            @keydown="(e: any) => handleMoneyInput(e, props.table.projectInfo.budgetMoney)"
      ...
      <div class="input-item">
        <label>项目启动时间：</label>
        <div class="input-area">
          <el-date-picker v-model="props.table.projectInfo.startTime" type="date" size="medium"
    ...
    <div class="button-container">
      <button class="save-btn" @click="saveProject(props.table)"
...
const validationStatus:any = reactive({
  ...
  coIntentionalProject: { isError: false, message: '' },
  coIntentionalUnit: { isError: false, message: '' }
});
...
const requiredFieldMap: Record<string, string> = {
  ...
  contact: '联系人',
  commander: '项目负责人(A/B)',
  coIntentionalProject: '合作意向项目',
  coIntentionalUnit: '合作意向单位'
};
