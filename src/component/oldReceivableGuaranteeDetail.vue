<template>
    ...
    <div class="input-section">
        <div class="input-grid">
            ...
            <div class="input-item">
                <div class="input-label">
                    <label><span class="required-mark">*</span>质保到期时间：</label>
                    <div v-if="validationStatus.guaranteeExpireTime.isError" class="error-message">{{ validationStatus.guaranteeExpireTime.message }}</div>
                    
                </div>
                <div class="input-area" :class="{ 'error-border': validationStatus.guaranteeExpireTime.isError }">
                    <el-date-picker v-model="props.table.projectInfo.guaranteeExpireTime" type="date" size="medium"
                        value-format="YYYY-MM-DD" :disabled="isOwner != undefined && !isOwner"
                        :prefix-icon="TimeIcon"></el-date-picker>
                </div>
            </div>
            <div class="input-item">
                <div class="input-label">
                    <label><span class="required-mark">*</span>质保金额（万元）：</label>
                    <div v-if="validationStatus.guaranteeMoney.isError" class="error-message">{{ validationStatus.guaranteeMoney.message }}</div>
                    
                </div>
                <div class="input-area" :class="{ 'error-border': validationStatus.guaranteeMoney.isError }">
                    <el-input v-model="props.table.projectInfo.guaranteeMoney" size="medium" type="number" :min="0"
                        @keydown="(e: any) => handleMoneyInput(e, props.table.projectInfo.guaranteeMoney)"
                        :disabled="isOwner != undefined && !isOwner"></el-input>
                </div>
            </div>
            <div class="input-item">
                <div class="input-label">
                    <label><span class="required-mark">*</span>履约保证金：</label>
                    
                    <div v-if="validationStatus.performanceBondMoney.isError" class="error-message">{{ validationStatus.performanceBondMoney.message }}</div>
                </div>
                <div class="input-area" :class="{ 'error-border': validationStatus.performanceBondMoney.isError }">
                    <el-input v-model="props.table.projectInfo.performanceBondMoney" size="medium" type="number"
                        :min="0"
                        @keydown="(e: any) => handleMoneyInput(e, props.table.projectInfo.performanceBondMoney)"
                        :disabled="isOwner != undefined && !isOwner"></el-input>
                </div>
            </div>
            <div class="input-item">
                <div class="input-label">
                    <label><span class="required-mark">*</span>履约保证金退回时间：</label>
                    <div v-if="validationStatus.backTime.isError" class="error-message">{{ validationStatus.backTime.message }}</div>
                    
                </div>
                <div class="input-area" :class="{ 'error-border': validationStatus.backTime.isError }">
                    <el-date-picker v-model="props.table.projectInfo.backTime" type="date" size="medium"
                        value-format="YYYY-MM-DD" :disabled="isOwner != undefined && !isOwner"
                        :prefix-icon="TimeIcon"></el-date-picker>
                </div>
            </div>
...
const validationStatus: any = reactive({
    ...
    guaranteeExpireTime: { isError: false, message: '' },
    guaranteeMoney: { isError: false, message: '' },
    performanceBondMoney: { isError: false, message: '' },
    backTime: { isError: false, message: '' }
});
...
const requiredFieldMap: Record<string, string> = {
    ...
    projectName: '项目名称',
    commander: '项目负责人(A/B)',
    guaranteeExpireTime: '质保到期时间',
    guaranteeMoney: '质保金额',
    performanceBondMoney: '履约保证金',
    backTime: '履约保证金退回时间'
};
...
function saveProject(table: any) {
    ...
    console.log('在代收质保金项目子组件中点击了保存');
    console.log('项目详情为：：：', table.projectInfo);
    // 检查必填项是否为空
    const missingFields: string[] = [];

    Object.keys(requiredFieldMap).forEach(field => {
    ...
    console.log('代收质保金项目生成的未填项为：：：', missingFields);


    // 如果有未填写的必填项
    if (missingFields.length > 0) {
        // 生成中文提示信息
        const chineseNames = missingFields.map(field => requiredFieldMap[field]);
...
.input-area :deep(.el-input .el-input__wrapper) {
    ...
    border: none;
    background: #ffffff;
    /* box-shadow: 0 0 0 1px #7c68ee inset; */
