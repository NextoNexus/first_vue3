<template>
    <div class="container">
        <div v-if="parentTables.length === 0" class="empty-state">
            <img src="./icon/暂无项目.png" alt="暂无项目" width="592.85px" height="442.03px">
            <div style="font-size: 48px; font-weight: 400; color: gray;"><span>暂无项目</span></div>
        </div>
            ...
            <div class="carousel-track" :style="{ transform: `translateX(${-currentIndex * 100}%)` }">
                <div v-for="(table, index) in parentTables" :key="table.id" class="parent-table" ref="ref_parentTables">
                    ...
                    <div class="custom-panel">
                        <div class="panel-content">
                            ...
                            <div class="action-area">
                                <el-button class="transfer-btn" @click="openDeliverForm(table)"
                                    :disabled="table.projectID == ''">
                                    项目转交
                                </el-button>
                                ...
                                <div class="task-control">
                                    <el-select v-model="currentAction" placeholder="更多操作" :suffix-icon='SelectIcon'
                                        ...
                                        <el-option label="下一步" value="complete"
                                            v-if="table.projectInfo.status != 'PAUSE' && table.projectID != '' && table.tasks[table.tasks.length - 1].status != 'FINISHED'"></el-option>
                                        <el-option :label="table.projectInfo.status == 'PAUSE' ? '项目启动' : '项目暂停'"
                                            :value="table.projectInfo.status == 'PAUSE' ? 'resume' : 'pause'"
                                            :disabled="table.projectID == ''"></el-option>
                                        <el-option label="项目结束" value="over"
                                            :disabled="table.projectInfo.status == 'PAUSE' || table.projectID == ''"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="table-content">
                        ...
                        <!-- 输入区域部分 -->
                        <EducationSystemADetail v-if="props.projectType == 'EDUCATION_SYSTEM_A_CUSTOMER'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <EducationSystemADetail v-if="props.projectType == 'EDUCATION_SYSTEM_B_CUSTOMER'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <EducationSystemADetail v-if="props.projectType == 'MEDICAL_SYSTEM_A_CUSTOMER'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <EducationSystemADetail v-if="props.projectType == 'MEDICAL_SYSTEM_B_CUSTOMER'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <EducationSystemADetail v-if="props.projectType == 'ORGAN_INSTITUTION_PROJECT'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <paymentCollectionDetail v-if="props.projectType == 'PAYMENT_COLLECTION_PROJECT'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <PartnerFollowUpDetail v-if="props.projectType == 'PARTNER_FOLLOW_UP'" :table="table"
                            :projectTypes="projectTypes" :isOwner="true" @saveProjectEvent="saveProject" />
                        <ReceivableGuaranteeDetail v-if="props.projectType == 'RECEIVABLE_GUARANTEE_PROJECT'"
                            :table="table" :projectTypes="projectTypes" :isOwner="true"
                            ...
                            <div class="app-container">
                                    ...
                                    <el-table :data="table.followRecords" @row-click="openDetailDialog">
                                        ...
                                        <el-table-column prop="nextPlan" label="下一步计划" :show-overflow-tooltip="true"
                                            ...
                                            <template #default="{ row }">
                                                ...
                                                <el-tooltip :content="row.nextPlan" placement="top"
                                        ...
                                        <el-table-column label="协调" align="center">
                                            <template #default="{ row }">
                                                <el-button class="coordination-btn" @click="openCoordination(row)"
                                                    :disabled="table.projectInfo.status == 'PAUSE'">
                                ...
                                <div class="footer-panel">
                                    <el-button class="add-btn" style="position: relative; left: 41px"
                                        @click="openAddForm"
                                        :disabled="table.projectID == '' || table.projectInfo.status == 'PAUSE'">
                                        + 新增
                                    </el-button>

                                    <el-pagination class="record-page-panel" layout="prev, pager, next"
        ...
        <!-- 协调管理对话框 -->
        <el-dialog v-model="coordinationDialogVisible" title="协调管理" width="1200px" height="900px"
        ...
        <!-- 添加新的协调表单对话框 -->
        <el-dialog v-model="newCoordinationVisible" title="新增协调记录" width="700px"
            style="border-radius: 10px solid black;">
            <el-form :model="newCoordination" class="new-coordination-form" label-width="80px">
                <label>协调时间</label>
                <el-form-item style="margin-bottom: 24px; margin-left: 0px;">
                    <div class="time-range" style="padding: 0px;">
                        <el-date-picker v-model="newCoordination.startTime" type="date" value-format="YYYY-MM-DD"
                ...
                <el-form-item style="margin-bottom: 24px">
                    <el-select v-model="newCoordination.depart" placeholder="请选择"
                        @change="(val: any) => handleDepartIDChange(val)" :suffix-icon='SelectIcon'>
                ...
                <label>协调人员</label>
                <el-form-item style="margin-bottom: 24px">
                    <el-select v-model="newCoordination.coUser" placeholder="请选择" :suffix-icon='SelectIcon'>
                ...
                <el-form :model="newRecord" label-width="0px" ref="addForm">
                    ...
                    <el-form-item class="record-input-area" :rules="timeRules" style="margin-bottom: 24px">
                        <div class="time-range-container">
                            <el-date-picker v-model="newRecord.startTime" type="date" value-format="YYYY-MM-DD HH:mm:ss"
                                format="YYYY-MM-DD" style="width: 222px" :prefix-icon="TimeIcon">
                            </el-date-picker>
                        </div>
                    </el-form-item>
                    <label for="">记录内容:</label>
                    <el-form-item class="record-input-area" prop="record" style="margin-bottom: 24px">
                        <el-input class="record" v-model="newRecord.record" type="textarea" :rows="3"
                            placeholder="请输入填写" :maxlength="textMaxLength" show-word-limit>
                        </el-input>
                    </el-form-item>
                    <label for="">预期目标:</label>
                    <el-form-item class="record-input-area" style="margin-bottom: 24px">
                        <el-input class="predict-target" v-model="newRecord.predictTarget" type="textarea" :rows="1"
                            placeholder="请输入填写" :maxlength="50" show-word-limit>
                        </el-input>
                    </el-form-item>
                    <label for="">下一步计划:</label>
                    <el-form-item class="record-input-area" style="margin-bottom: 24px">
                        <el-input class="next-plan" v-model="newRecord.nextPlan" type="textarea" :rows="3"
                            placeholder="请输入填写" :maxlength="textMaxLength" show-word-limit>
                        </el-input>
                    </el-form-item>
                    <!-- 其他表单项保持类似结构 -->
        ...
        <el-dialog v-model="detailDialogVisible" title="跟进记录" width="800px" class="rounded-dialog">
            <div class="form-container">
                ...
                <el-form :model="currentRecordDetail" label-width="0px" ref="addForm" :disabled="!isEditing">
                    ...
                    <el-form-item class="record-input-area" :rules="timeRules" style="margin-bottom: 24px">
                        <div class="time-range-container">
                            <el-date-picker v-model="currentRecordDetail.startTime" type="date"
                                value-format="YYYY-MM-DD" format="YYYY-MM-DD" style="width: 222px"
                    ...
                    <label for="">记录内容</label>
                    <el-form-item class="record-input-area" prop="record" style="margin-bottom: 24px">
                        <el-input class="record" v-model="currentRecordDetail.record" type="textarea" :rows="3"
                            placeholder="请输入填写" :maxlength="textMaxLength" show-word-limit>
                        </el-input>
                    </el-form-item>
                    <label for="">预期目标</label>
                    <el-form-item class="record-input-area" style="margin-bottom: 24px">
                        <el-input class="predict-target" v-model="currentRecordDetail.predictTarget" type="textarea"
                            :rows="2" placeholder="请输入填写" :maxlength="50" show-word-limit>
                        </el-input>
                    </el-form-item>
                    <label for="">下一步计划</label>
                    <el-form-item class="record-input-area" style="margin-bottom: 24px">
                        <el-input class="next-plan" v-model="currentRecordDetail.nextPlan" type="textarea" :rows="2"
                            placeholder="请输入填写" :maxlength="textMaxLength" show-word-limit>
                        </el-input>
                    </el-form-item>
                </el-form>
        ...
        <el-dialog v-model="deliverDialogVisible" title="项目转交" width="500px" class="rounded-dialog">
            ...
            <div class="form-container">
                <el-form :model="deliverRecord" class="deliver-form" label-width="0px" ref="deliverForm">
                    ...
                    <label>转交日期：</label>
                    <el-form-item prop="transmitTime" style="margin-bottom: 24px">
                        <el-date-picker v-model="deliverRecord.transmitTime" type="date" value-format="YYYY-MM-DD"
                            class="deliver-time" format="YYYY-MM-DD" :prefix-icon="DateIcon"
        ...
        <div class="dynamic-pagination" v-if="parentTables.length > 0">
            <el-pagination background layout="prev, pager, next" :total="parentTables.length" :page-size="1"
                :current-page="currentIndex + 1" @current-change="handlePaginationChange" />
        </div>
...
import { getEmployeesByDept } from "@/api/website/my_office/Workspace";

import { searchProject } from "@/api/website";
...
function handleActionChange(tableIndex: any) {
    ...
    if (currentAction.value === "complete") {
        ...
        if (currentTaskIndex < 0) {
            ElMessage.success('没有正在进行的项目，无需下一步。')
            return
        }
        ...
        getProgressNext(DTO).then((res: any) => {
            if (res.code == 200) {
                ...
                // 添加任务流程自动居中调用
                // nextTick(() => {
                //     centerActiveTaskInFlow(tableIndex);
                // });
            } else {
                // console.log(res.msg);
            ...
            .then(() => {
                ...
                updateProjectStatus(dto).then((res: any) => {
                    if (res.code == 200) {
                        ...
                        if (parentTables.value.length - 1 > 0) {
...
const saveNewCoordination = () => {
    // 验证逻辑...
    // 验证逻辑...
    if (newCoordination.coUser == currentUser.value) {
        console.log(newCoordination.coUser, currentUser.value);
    ...
    const tableIndex = parentTables.value.findIndex(
        (table: any) => table.projectID == currentRecord.value.projectId
    );
    ...
    addProjectCoordinate(coordinationDTO).then((res: any) => {
        if (res.code == 200) {
            ...
            // 保存成功后刷新协调管理表
            getProjectCoordinate(currentRecord.value.id).then((res: any) => {
                if (res.code == 200) {
                    coordinationData.value = [...res.data];
                    coordinationDialogVisible.value = true;
                }
...
const saveNewRecord = (index: any) => {
    ...
    if (!newRecord.startTime || !newRecord.record) {
...
function openDeliverForm(table: any) {
    ...
    deliverRecord.projectProgress = doingIndex > 0 ? table.tasks[doingIndex].content : "全部完成";
    deliverRecord.userName = "";
    deliverRecord.userID = "";
    deliverRecord.transmitTime = now.format("YYYY-MM-DD");
        ...
        if (res.code == 200) {
            ...
            getUsersByDepartID(1, 9999, saleDepID).then((res: any) => {
                if (res.code == 200) {
                    ...
                    // 过滤掉当前登录用户
                    const filteredUsers = res.data.list.filter((user: any) => {
                        // 比较用户ID，排除当前登录用户
                        return user.id !== currentUserId;
                    });

                    // 赋值过滤后的用户列表
                    personOptions.value = [...filteredUsers];
                } else {
                    ElMessage.warning(res.msg);
                }
            });
...
function saveDeliverRecord(index: any) {
    console.log('deliverRecord', deliverRecord);
    if (deliverRecord.userID == "") {

        ElMessage.warning('请输入转交同事。')
        return
    }
    const preParentTablesLength = parentTables.value.length;
    let currentProjectID = parentTables.value[index].projectID;
...
function saveProject(table: any) {
    ...
    const transformedData = computed(() => {
        const result = { ...table.projectInfo };
    ...
    if (table.projectID == '') {
        ...
        const dto = {
            projectDTO: transformedData.value,
...
function addParentTable() {
    console.log('AllProjectRouter.vue中执行了addParentTable方法');
...
const handlePaginationChange = (newPage: number) => {
    currentIndex.value = newPage - 1; // 转换为从0开始的索引
};
    ...
    getProjectId(projectType).then((res: any) => {
        if (res.code == 200) {
            ...
            projectIDs.value.forEach((element: any, index: any) => {
                ...
                const tableindex = parentTables.value.findIndex(
                    (table: any) => table.projectID == element
                );
                parentTables.value.sort((a: any, b: any) => a.id - b.id)
                console.log('排序完成的parentTables集合为》》', parentTables.value);
                ...
                refreshRecord(
                    ...
                    "start_time desc",
                    parentTables.value[tableindex].projectID
                );
                refreshChangeRecord(
...
function refreshProgress(projectID: string, projectType: string) {
    getProjectProgressList(projectID, projectType).then((res: any) => {
        if (res.code == 200) {
            console.log("processedResDate", res.data);

            const index = parentTables.value.findIndex(
                (table: any) => table.projectID == projectID
            );
            parentTables.value[index].tasks = [...res.data];
            projectProgress.value.push(res.data);
            // updateAllConnectors();
    ...
    getProjectRecordList(pageNum, pageSize, orderBy, projectID).then(
        (res: any) => {
            if (res.code == 200) {
                ...
                // console.log('刷新跟进记录后，该parentTables为：：：',parentTables.value[index]);
...
watch(
    ...
    (newProjectType, oldProjectType) => {
        console.log('项目类型发生变化，从', oldProjectType, '变为', newProjectType);
        if (newProjectType) {
            currentIndex.value = 0
            refreshProjectAllData(newProjectType);
        }
    }
);
...
// 初始化连接线
onMounted(() => {
    console.log('AllProjectRouter.vue组件初始化完毕，props.projectType的值为：：', props.projectType);
...
.dynamic-pagination:deep(.el-pagination.is-background .el-pager li) {
    width: 120px;
    height: 24px;
    border-radius: 4px;
    border-width: 1px;
...
.dynamic-pagination:deep(.el-pagination.is-background .el-pager li.is-active) {
    width: 120px;
    height: 24px;
    border-radius: 4px;
    border-width: 1px;
...
.dynamic-pagination:deep(.el-pager){
    gap: 40px;
}
...
.new-coordination-form :deep(.el-form-item__content) {
...
.deliver-form :deep(.el-input__suffix) {
    position: relative;
    left: -24px;
}
...
.deliver-form :deep(.el-date-editor) {
    border-radius: 4px;
    width: 100% !important;
    height: 100% !important;
}

.deliver-form :deep(.el-date-editor .el-input__prefix) {
    margin-left: .24rem;
}
...
/* 确保转交同事下拉框和转交日期选择器保持100%不透明度 */
.deliver-form :deep(.el-select .el-input__wrapper),
.deliver-form :deep(.el-date-editor .el-input__wrapper) {
    opacity: 1 !important;
}

.deliver-form :deep(.el-select .el-input__inner) {
    font-size: 18px;
    font-weight: 400px;
    padding-left: 16px;
}

.deliver-form :deep(.el-input__wrapper .el-input__inner) {
    font-size: 18px;
    font-weight: 400px;
    padding-left: 16px;
}

/* 强化项目转交表单中下拉框样式，确保在所有环境下正确显示 */
.deliver-form :deep(.el-select .el-input__wrapper) {
    height: 40px !important;
...
/* 确保下拉箭头正确显示 */
.deliver-form :deep(.el-select .el-input__suffix) {
    height: 40px !important;
}

.deliver-form :deep(.el-select .el-icon) {
    height: 40px !important;
    line-height: 40px !important;
}

/* .deliver-time :deep(.el-input__wrapper .el-input__inner) {
    font-size: 18px;
    font-weight: 400px;
    padding-left: .24rem;
...
/* .deliver-time :deep(.el-input__wrapper) {
    width: 100%;
    height: 100%;
}

.deliver-time :deep(.el-input__prefix){
    margin-left: .24rem;
} */
...
<style scoped>
/* edge浏览器 */
.task-control :deep(.el-select__wrapper) {
...
.task-control :deep(.el-select__placeholder) {
    color: #333333;
    font-size: 20px;
    font-weight: 400;
}
...
/* 浏览器通用 */
.new-coordination-form :deep(.el-select__wrapper) {
    border: 1px solid #7c68ee;
    box-shadow: none;
}
...
.deliver-form :deep(.el-select__wrapper) {
    border: 1px solid #7c68ee;
    box-shadow: none;
}


:deep(.record .el-textarea__inner) {
    height: 96px;
}

:deep(.predict-target .el-textarea__inner) {
    height: 40px;
}

:deep(.next-plan .el-textarea__inner) {
    height: 96px;
}
